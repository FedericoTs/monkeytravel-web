-- get_live_trip_baseline(): post-trip return must not be clipped by the window.
--
-- 20260905160100 pulled the window's human page views into a temp table and
-- had the post-trip check read it. But the trips counted for that metric are
-- those whose 7-day post-trip window CLOSED inside [lo, hi), i.e. trips that
-- ended in the seven days before lo have part of their return window before
-- the cut and their owner's opens were not in the temp table. Measured: the
-- same window read 10.7% from the view and 9.5% from the temp table (8 vs 9
-- of 84 trips). Small number, real bug.
--
-- The check now reads page_views_human directly. It is ~84 lookups on
-- idx_page_views_user_id, so it costs nothing; the temp table remains the
-- source for everything that is genuinely window-bounded.
