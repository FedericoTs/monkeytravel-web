...
-- security branch.
--
-- Now: at least 4 trips per city whatever the caller asks, and at most 50
-- rows. The body is otherwise unchanged from 20260826231948.
...
as $function$
  ...
  from counts c
  where c.trips_all >= greatest(coalesce(p_min_trips, 4), 4)
