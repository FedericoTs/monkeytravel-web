...
export default function ExportMenu({ trip, destination, meta, coverImageUrl, galleryPhotos }: ExportMenuProps) {
  ...
  const handleGoogleCalendar = () => {
    ...
    if (firstActivity && firstDay) {
      ...
      void openExternal(`https://calendar.google.com/calendar/render?${params.toString()}`);
    }
    ...
    <div className="relative" ref={menuRef}>
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-64 bg-white border border-slate-200 rounded-xl shadow-lg py-2 z-20">
          ...
          {/*
            Apple Calendar entry removed (cycle covering #235): the
            previous href pointed at a `webcal://monkeytravel.app/api/export/calendar/${title}`
            route that never existed in the codebase (404 in prod). The
            in-dropdown "Calendar File (.ics)" download already works in
            Apple Calendar — users just double-click the file. If we
            ever wire a real webcal subscription feed we can restore a
            dedicated entry then.
          */}
        </div>
      )}
    </div>
