...
- **Every `page_views` row before 2026-09-17 over-counts multi-page sessions about 3x.** Next 16 prefetches every visible link, and the guard meant to skip prefetches checked `next-router-prefetch`, a header Next's own middleware adapter deletes (with `rsc` and the rest of the Flight headers) before middleware runs, so it never fired: each hover or viewport prefetch became a "view" (`/blog → /` 90 ms apart, all nav-menu targets; 4,377 of 5,359 consecutive same-session views on 2026-09-16 were under 500 ms apart). Sessions and visitors are unaffected; views per session before that date are not comparable with after. Since then the middleware counts browser navigations only (`sec-fetch-dest: document`, the one signal Next does not strip: `lib/analytics/page-view-classifier.ts`) and `components/analytics/PageViewBeacon.tsx` records in-app navigations through `/api/page-view`, which a prefetch can never trigger. Every response carries `x-mt-pv: counted | counted;dry | skip:<why>` so a preview deployment can be probed with curl.
