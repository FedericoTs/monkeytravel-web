...
- **Every `page_views` row before 2026-09-17 over-counts multi-page sessions about 3x.** Next 16 prefetches every visible link, and the guard meant to skip prefetches checked a header Vercel never forwards to the middleware, so each hover or viewport prefetch became a "view" (`/blog → /` 90 ms apart, all nav-menu targets; 4,377 of 5,359 consecutive same-session views on 2026-09-16 were under 500 ms apart). Sessions and visitors are unaffected; views per session before that date are not comparable with after. The rule now lives in `lib/analytics/page-view-classifier.ts`, and the middleware answers every response with `x-mt-pv: counted | counted;dry | skip:<why>` so a preview deployment can be probed with curl.
