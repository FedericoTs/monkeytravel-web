...
| 0.2 | **Human view over `page_views`** | Do **not** widen `bot-detection.ts` (it is correctly UA-only and documents why). Add a SQL view `page_views_human` and a nightly job that stamps `page_views.is_automation` from session-level signals: ≥20 views in a day, ≥8 views/hour sustained from one city with <3% click-through, and the CN/SG/HK-datacenter + desktop + zero-engagement pattern. Labelling only — no request is ever refused. Admin dashboard and every script switch to the view. |
| 0.3 | **Retire `collaborator_ids`** | Zero references. Backfill nothing; make `trip_collaborators` the single source of truth; drop the column in a later migration once Phase 2's `trip_participants` exists (one schema change, not two). |
