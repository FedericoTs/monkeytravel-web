...
| **Standalone `/multi-city`** | orphaned | Save-less "Preview" page, **zero inbound links**. The real multi-city entry is the wizard (RC1). | Delete or 301→`/trips/new` (avoid a dead, save-less duplicate). `multi-city/page.tsx:4,9-11,116-118` |
| **`/welcome`** | dead code | Retired stub that just `redirect("/trips/new")`; `WelcomeClient.tsx` imported nowhere; residual `router.push("/welcome")` in login. | Delete `app/[locale]/welcome/`, remove login branch `login/page.tsx:108-112`, repoint profile "join waitlist" `ProfileClient.tsx:982`, drop from `robots.ts`. |
