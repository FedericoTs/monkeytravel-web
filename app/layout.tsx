...
export const metadata: Metadata = {
  ...
  //
  // **2026-05-30 fix**: GSC audit found the og:image meta tag wasn't emitting
  // sitewide. Root cause: when you define an explicit `openGraph` object in
  // metadata, Next.js's file convention (app/opengraph-image.tsx) doesn't
  // auto-merge into it — the meta tag silently goes missing. Compounded by:
  // the dynamic /opengraph-image and /twitter-image routes ALSO return 404 in
  // production (even to facebookexternalhit), so even if the meta tag pointed
  // there, the social-preview fetch would fail. Fix is to wire both og + twitter
  // to the static /og-image.png file (1200x630, 118KB, served via /public).
  // Result: Facebook/LinkedIn/WhatsApp/Slack/Twitter previews now show the
  // branded card instead of plain text.
  openGraph: {
