      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: 'MonkeyTravel — AI Itinerary Generator',
        },
      ],
    },
    twitter: {
...
const COMPARISON_KEYS = ['dayByDay', 'realVenues', 'timeBlocks', 'budgetOptions', 'exportOptions'] as const;
const FAQ_KEYS = ['howFast', 'howAccurate', 'multiCity', 'exportFormats', 'isFree'] as const;

export default async function AIItineraryGeneratorPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations('aiItineraryGenerator');
  const prefix = locale === 'en' ? '' : `/${locale}`;
  ...
  const breadcrumbItems = [
    ...
    {
      name: t('breadcrumbs.aiItineraryGenerator'),
      url: `${BASE_URL}${prefix}${PAGE_PATH}`,
    },
  ];
  ...
  return (
    <>
      ...
      <main className="min-h-screen">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex flex-wrap items-center justify-center gap-8 sm:gap-16 text-center">
              <div>
                ...
                <div className="text-sm text-[var(--foreground-muted)]">{t('stats.generationTime')}</div>
              </div>
              <div className="hidden sm:block w-px h-12 bg-gray-200" />
              <div>
                <div className="text-3xl sm:text-4xl font-bold text-[var(--primary-ink)]">180+</div>
                <div className="text-sm text-[var(--foreground-muted)]">{t('stats.destinations')}</div>
              </div>
              <div className="hidden sm:block w-px h-12 bg-gray-200" />
              <div>
                <div className="text-3xl sm:text-4xl font-bold text-[var(--primary-ink)]">100%</div>
                <div className="text-sm text-[var(--foreground-muted)]">{t('stats.free')}</div>
              </div>
              <div className="hidden sm:block w-px h-12 bg-gray-200" />
              <div>
                <div className="text-3xl sm:text-4xl font-bold text-[var(--primary-ink)]">3</div>
                <div className="text-sm text-[var(--foreground-muted)]">{t('stats.budgetTiers')}</div>
              </div>
            </div>
          </div>
        </section>
        ...
        {/* ================================================================
            COMPARISON TABLE (MonkeyTravel vs ChatGPT/Gemini)
            ================================================================ */}
          ...
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm">
              ...
              <div className="grid grid-cols-3 border-b border-gray-200 bg-[var(--background-alt)] text-sm font-semibold">
                ...
                <div className="border-l border-gray-200 p-4 text-center text-[var(--primary-ink)]">
                  MonkeyTravel
                </div>
            ...
            <div className="relative overflow-hidden rounded-[2.5rem] mesh-gradient p-12 sm:p-16 lg:p-20">
              ...
              <div className="relative text-center">
                ...
                <div className="pt-8 border-t border-white/10 max-w-md mx-auto">
                  ...
                  <EmailSubscribe variant="dark" source="ai-itinerary-generator-cta" />
                </div>
              </div>
            </div>
          </div>
