...
export default function ProfileClient({ profile: initialProfile, stats, betaAccess: initialBetaAccess }: ProfileClientProps) {
  ...
  const handleExportData = async () => {
    ...
    try {
      ...
      const filename = filenameMatch ? filenameMatch[1] : "monkeytravel-data-export.json";
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-left">
                    <span className="font-medium text-slate-700 block">
                      {isExporting ? t("dataExport.downloading") : t("dataExport.button")}
                    </span>
                    <span className="text-xs text-slate-500">{t("dataExport.includes")}</span>
                  </div>
                </div>
                {isExporting ? (
